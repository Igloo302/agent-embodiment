...
**功能增强**
- ✨ 被动学习支持操作学习：Agent 在使用设备上的软件/服务时，自动从返回信息中提取硬件能力
  - SSH 操作后提取：GPU 型号、显存、OS、架构、Docker/PVE 能力
  - API 调用后提取：Ollama/ComfyUI/vLLM/DSM 返回的 GPU 和能力信息

**Bug 修复**
- 🐛 修复 `discover-network.sh` 重复输出表头导致解析失败的问题
- 🐛 修复所有脚本硬编码 `~/.hermes/skills/agent-embodiment/` 路径，改为动态获取

**文档**
- 📝 SKILL.md 路径改为 `<SKILL_DIR>` 占位符，适配不同 Agent 软件
- 📝 README.md 更新特性列表，新增被动学习和路径无关说明

### v1.0.0 (2026-05-02)

**核心功能**
- 🎉 MAC 地址作为设备唯一标识（替代 IP-based ID）
- 🎉 MCP 工具精简为 2 个：`query_device` + `learn_device`
- 🎉 完整发现流程：本机 + 网络 + 推理 + 硬件
- 🎉 被动学习：从对话中自动识别设备信息
